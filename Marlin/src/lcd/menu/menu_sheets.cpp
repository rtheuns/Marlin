...
#if HAS_MARLINUI_MENU && ENABLED(SHEETS_FEATURE)
...
void menu_sheets() {
  ...
  ACTION_ITEM_F(F(activesheet_str), []{ sheets.set_next_active_sheet(); menu_sheets(); });

  // Create list of sheet submenu items
  for (uint8_t i = 0; i < MAX_SHEETS; i++) {
    loop_sheet_index = i;
    SUBMENU_F(F(sheets.settings.sheets_list[i].name), []{ menu_sheet(loop_sheet_index); });
  }
